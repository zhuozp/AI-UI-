...
flowchart TB
    %% ========== 用户交互层 ==========
    subgraph L1 ["🎯 L1 - 用户交互层"]
        direction LR
        subgraph DEV_UI["开发交互"]
            UI1["🖥️ Cursor IDE<br/>开发者界面"]
            UI2["⚙️ 模型配置器<br/>AI模型选择"]
        end
        subgraph TEST_UI["测试交互"] 
            UI3["💻 CLI脚本<br/>批量执行"]
        end
    end
    
    %% ========== AI处理核心 ==========
    subgraph L2 ["🧠 L2 - AI处理核心"]
        direction LR
        subgraph AI_CORE["智能决策中心"]
            AI1["🤖 AI调度中心<br/>统一模型管理"]
        end
        subgraph AI_MODELS["AI模型服务"]
            AI2["👁️ Gemini-2.5-Pro<br/>视觉理解模型"] 
            AI3["💬 GPT-4.0<br/>自然语言处理"]
        end
    end
    
    %% ========== 双模式执行引擎 ==========
    subgraph L3 ["⚡ L3 - 双模式执行引擎"]
        direction LR
        subgraph DEV_ENGINE["开发阶段引擎"]
            ENG1["📡 Mobile-MCP Server<br/>实时交互处理"]
            ENG2["🔄 指令解析器<br/>自然语言理解"]
        end
        subgraph PROD_ENGINE["回归阶段引擎"]
            ENG3["🚀 扩展Midscene框架<br/>批量自动化"]
            ENG4["🎯 混合定位引擎<br/>结构化+视觉"]
        end
    end
    
    %% ========== 统一设备操作层 ==========
    subgraph L4 ["🔧 L4 - 统一设备操作层"]
        direction LR
        subgraph DEVICE_OPS["设备操作"]
            DEV1["📱 设备管理器<br/>跨平台统一接口"]
            DEV2["🌲 UI解析引擎<br/>界面结构提取"]
        end
        subgraph DEVICE_MONITOR["设备监控"]
            DEV3["📊 状态监控器<br/>实时设备状态"]
        end
    end
    
    %% ========== 设备连接层 ==========
    subgraph L5 ["📱 L5 - 设备连接层"]
        direction LR
        subgraph DRIVERS["设备驱动"]
            DRV1["🤖 ADB服务<br/>Android通信"]
            DRV2["🍎 WebDriverAgent<br/>iOS通信"]
        end
    ...
    %% ========== 本地存储层 ==========
    subgraph L6 ["💾 L6 - 本地存储层"]
        direction LR
        subgraph STORAGE_SYS["存储系统"]
            ST1["📋 测试结果<br/>执行记录+截图"] 
            ST2["⚙️ 配置管理<br/>系统参数+密钥"]
            ST3["🗄️ 智能缓存<br/>AI分析结果"]
        end
    end
    
    %% ========== 层级样式定义 ==========
    %% L1 - 用户交互层 (蓝色渐变)
    style L1 fill:#e3f2fd,stroke:#1976d2,stroke-width:3px,color:#000
    style UI1 fill:#bbdefb,stroke:#1565c0,stroke-width:2px,color:#000
    style UI2 fill:#90caf9,stroke:#1565c0,stroke-width:2px,color:#000
    style UI3 fill:#64b5f6,stroke:#1565c0,stroke-width:2px,color:#000
    
    %% L2 - AI处理核心 (橙色渐变)
    style L2 fill:#fff3e0,stroke:#f57c00,stroke-width:3px,color:#000
    style AI1 fill:#ffcc02,stroke:#ef6c00,stroke-width:2px,color:#000
    style AI2 fill:#ffb74d,stroke:#ef6c00,stroke-width:2px,color:#000
    style AI3 fill:#ffa726,stroke:#ef6c00,stroke-width:2px,color:#000
    
    %% L3 - 双模式执行引擎 (绿紫渐变)
    style L3 fill:#f1f8e9,stroke:#689f38,stroke-width:3px,color:#000
    style ENG1 fill:#c8e6c9,stroke:#558b2f,stroke-width:2px,color:#000
    style ENG2 fill:#a5d6a7,stroke:#558b2f,stroke-width:2px,color:#000
    style ENG3 fill:#e1bee7,stroke:#8e24aa,stroke-width:2px,color:#000
    style ENG4 fill:#ce93d8,stroke:#8e24aa,stroke-width:2px,color:#000
    
    %% L4 - 统一设备操作层 (青色渐变)
    style L4 fill:#e0f2f1,stroke:#00695c,stroke-width:3px,color:#000
    style DEV1 fill:#b2dfdb,stroke:#004d40,stroke-width:2px,color:#000
    style DEV2 fill:#80cbc4,stroke:#004d40,stroke-width:2px,color:#000
    style DEV3 fill:#4db6ac,stroke:#004d40,stroke-width:2px,color:#000
    
    %% L5 - 设备连接层 (紫色渐变)
    style L5 fill:#f3e5f5,stroke:#7b1fa2,stroke-width:3px,color:#000
    style DRV1 fill:#ce93d8,stroke:#6a1b9a,stroke-width:2px,color:#000
    style DRV2 fill:#ba68c8,stroke:#6a1b9a,stroke-width:2px,color:#000
    style HW1 fill:#ab47bc,stroke:#6a1b9a,stroke-width:2px,color:#000
    style HW2 fill:#9c27b0,stroke:#6a1b9a,stroke-width:2px,color:#000
    
    %% L6 - 本地存储层 (灰蓝渐变)
    style L6 fill:#eceff1,stroke:#455a64,stroke-width:3px,color:#000
    style ST1 fill:#cfd8dc,stroke:#37474f,stroke-width:2px,color:#000
    style ST2 fill:#b0bec5,stroke:#37474f,stroke-width:2px,color:#000
    style ST3 fill:#90a4ae,stroke:#37474f,stroke-width:2px,color:#000
```
...
|------|------|----------|----------|----------|
| **🎯 L1** | 用户交互层 | 统一用户入口 | Cursor IDE + CLI、模型配置 | 🔵 蓝色系 |
| **🧠 L2** | AI处理核心 | 智能决策中心 | AI调度中心、Gemini-2.5-Pro、GPT-4.0 | 🟠 橙色系 |
| **⚡ L3** | 双模式执行引擎 | 业务逻辑处理 | 开发模式(Mobile-MCP)、回归模式(扩展Midscene) | 🟢🟣 绿紫系 |
| **🔧 L4** | 统一设备操作层 | 设备控制抽象 | 设备管理器、UI解析引擎 | 🔵 天蓝系 |
| **📱 L5** | 设备连接层 | 底层设备通信 | ADB + WebDriverAgent、Android/iOS设备 | ⚫ 灰色系 |
| **💾 L6** | 本地存储 | 数据持久化 | 配置 + 结果 + 缓存 | 🔘 蓝灰系 |
...
```mermaid
flowchart LR
    A[用户指令] --> B[双模式引擎] --> C[AI处理] --> D[设备操作] --> E[结果存储]

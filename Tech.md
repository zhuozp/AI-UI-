...
sequenceDiagram
    ...
    participant Vision as 视觉理解
    participant Device as 设备操作
    
    User->>AI: "点击登录按钮"
    AI->>Device: 获取屏幕截图
    Device-->>AI: 返回截图和结构化数据
    AI->>Vision: 分析屏幕内容
    Vision-->>AI: 返回元素位置和置信度
    AI->>Device: 执行点击操作
    Device-->>AI: 返回操作结果
    AI->>Vision: 验证操作效果
    Vision-->>AI: 确认操作成功
